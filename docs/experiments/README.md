...
| [E14 — nested enforcement](E14-groups-and-boundaries.md) | Does nested (group-scoped) enforcement behave differently from flat enforcement, and does the near-optimal set grow with group-count complexity? | **No — it shrinks, not grows** (1/2 → 1/3 → 1/5 of configurations pass as group count rises; count stays flat at 1); partial coverage can be worse than none until a sharp tipping point |
| [E15 — boundaries](E15-boundaries.md) | Does closing the community to outsiders matter, on top of nested enforcement (E14)? | **Yes, but conditionally** — boundaries dominate against a genuinely greedy outsider (near-optimal set empty at every complexity level), but cost nothing against an outsider that would restrain itself anyway |
